...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def mouseReleaseEvent(self, event):
        ...
        prev_sel = self._prev_selection_nodes
        self._previous_pos = event.pos()

        post_sel = self.selected_nodes()
        selected = [n.id for n in post_sel if n not in prev_sel],
        deselected = [n.id for n in prev_sel if n not in post_sel]
        if selected or deselected:
            self.node_selection_changed.emit(selected, deselected)
    ...
    def sceneMousePressEvent(self, event):
        ...
        if node_items:
            ...
            if event.button() == QtCore.Qt.LeftButton:
                ...
                self.node_selection_changed.emit([node.id], [])
